...
void TwoPlayersGame(const unsigned char* first_word, Player* Player_1, Player* Player_2);
//  =============================================  Интерфейс игры  =====================================================
...
void Game_ChangeCell(int KEY, int* current_X, int* current_Y, unsigned char playing_field[5][5]);
...
int main() {
    Main_Data.Data = DataFile_OpenDataFile();  //  Открытие файла и запись основных данных в структуру Main_Data
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_UP) {  //  Передвижение вверх
                if (current_Y > 0 && current_option == PLAY_FIELD) {  //  Передвижение по игровому полю
                    Game_ChangeCell(code, &current_X, &current_Y, playing_field);
                    continue;
            ...
            if (code == KEY_ENTER) {
                if (current_option == PLAY_FIELD) {  //  Выбор ячейки на игровом поле
                    ...
                    if (current_X == new_X && current_Y == new_Y && mode == 1 && Game_SecondChance_DeleteFromNotepad()) {  //  Если выбрана выставленная только что буква
                    ...
                    if (((turn == 0 && Player_1->Skips != 3) || turn == 1 && Player_2->Skips != 3) &&
                            Game_SecondChance_SKIP()) {  //  Подтвержден и доступен
