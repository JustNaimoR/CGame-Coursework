...
int main() {
    /*con_init(80, 25);
    Sleep(1500);  //  Усыпляем компилятор

    Player* player_1 = malloc(sizeof (struct Player));
    Player* player_2 = malloc(sizeof (struct Player));

    player_1->Number_of_points = 90;
    player_2->Number_of_points = 87;

    player_1->Name = (unsigned char*) "Игрок 1";
    player_2->Name = (unsigned char*) "Игрок 2";


    Game_EndOfGame(player_1, player_2);
    return 0;*/
...
void Game_AddWordInList(unsigned char* word) {
    ///  Функция добавления нового слова в список всех слов (Возвращает адрес на новый первый элемент списка)
...
void Game_EndOfGame(Player* Player_1, Player* Player_2) {
    ...
    //TODO  создать запись в таблицу рекордов
