...
int main() {
    ...
    unsigned char pole[5][5] = { { 0,0,224,0,0 }, { 0,0,226,0,0 }, { 0,0,242,0,0 }, { 0,0,238,0,0 }, { 0,0,240,0,0 } };
    COMP_COMPStep(pole, 0);
    return 0;
...
unsigned char* COMP_COMPStep(unsigned char playing_field[5][5], Difficulty_level difficult) {
    ...
    //TODO  из полученных чисел не учитывать использованные ранее (сделать удаление)

    //  Получаем количество каждого размера слов
    int koll[30];  //  Массив количества каждой длины полученных слов
    memset(koll, 0, sizeof (koll));
    //  Подсчет количества
    for (int mark = 0; words[mark][0] != 0; mark++) {
        koll[strlen((char*) words[mark])]++;
    }
