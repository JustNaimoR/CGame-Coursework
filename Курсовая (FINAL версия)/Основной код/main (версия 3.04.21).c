...
//  Перечисляемый тип для уровня сложности игры
...
void Help_Output(int option) {
    ...
    char Strings_1[21][120] = {"                        [?] Справка [?]                       ",
                               ...
                               "+------------------------------------------------------------+",
                               "                                                              "  };
    char String_2[9][120] = {  "                   ==== "," Стереть рекорды "," ====                ",
                               "                                                              ",
                               "                   ==== "," Анимация в игре "," ====               ",
                               "                                                              ",
                               "+------------------------------------------------------------+"  };
    const int width_of_picture = 62;  //  Ширина всей картинки меню
    const int height_of_picture_1 = 21;  //  Высота первой части меню
    const int height_of_picture_2 = 9;  //  высота второй части меню (с учетом разделения на 3 части - смотри массив)
    ...
    for (int string_2 = 0; string_2 < height_of_picture_2; string_2++) {
        ...
        if (string_2 == 1 && option == 0 || string_2 == 5 && option == 1) {  //  Услоиве окраса
            con_set_color(CON_CLR_BLACK, CON_CLR_WHITE_LIGHT);  //  Меняем на белый фон с черными буквами
...
//  =================================================  Игра  ===========================================================
void Game(enum Difficulty_level difficulty) {
    ///  Основная функция игры  (difficulty - сложность игры с роботом. Значение 0 при игре 2 игроков)
...
void Option_Help() {
    ...
    while (1) {
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_DOWN && current_option < 1) {  //  Если клавиша вниз
                current_option++;
                continue;
            }
...
void Save_data_in_file() {
    ...
    fprintf(file, "Random %d\nAnimation %d\n", Main_Data.Random, Main_Data.Animation);
...
void Record_the_data_in_struct(char* Data) {
    ...
    char* point = strstr(Data, "Random");  //  Запись адреса на тип Random в файле
