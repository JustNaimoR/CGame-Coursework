...
void Game_players(const unsigned char* first_word, Player* Player_1, Player* Player_2) {
    ...
    while (1) {
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_UP) {
                if (current_Y > 0 && current_option == PLAY_FIELD) {  //  Переход к новой ячейке на поле
                    current_Y--;
                    continue;
                } else if (current_option == CHECK_WORD) {  //  Подсвет функции "блокнот" с функции "Проверить слово"
                    current_option = NOTEPAD;
                    continue;
                }
            }
            if (code == KEY_DOWN) {
                if (current_Y < 4 && current_option == PLAY_FIELD) {  //  Переход к новой ячейке на поле
                    current_Y++;
                    continue;
                } else if  (current_option == NOTEPAD) {
                    current_option = CHECK_WORD;
                    continue;
                }
            }
            if (code == KEY_LEFT) {
                if (current_X > 0 && current_option == PLAY_FIELD) {  //  Переход к новой ячейке на поле
                    current_X--;
                    continue;
                } else if (current_X == 0 && current_option == PLAY_FIELD) {  //  Подсвет функции "Skip"
                    current_option = SKIP;
                    continue;
                } else if (current_option == NOTEPAD || current_option == CHECK_WORD) {  //  Переход к полю
                    current_option = PLAY_FIELD;
                    continue;
                }
            }
            if (code == KEY_RIGHT) {
                if (current_X < 4 && current_option == PLAY_FIELD) {  //  Переход к новой ячейке на поле
                    current_X++;
                    continue;
                } else if (current_option == SKIP) {  //  Переход к игровому полю с функции "Skip"
                    current_option = PLAY_FIELD;
                    continue;
                } else if (current_option == PLAY_FIELD && current_X == 4 && current_Y <= 3) {  //  Подсвет функции "блокнот"
                    current_option = NOTEPAD;
                    continue;
                } else if (current_option == PLAY_FIELD && current_X == 4 && current_Y == 4) {  //  Подсвет функции "Проверить слово"
                    current_option = CHECK_WORD;
                    continue;
                }
            }
            if (code == KEY_ENTER) {
                if (current_option == PLAY_FIELD) {
                    //  Подсвет ячейки и вставка слова
                } else if (current_option == SKIP) {  //  Опция пропуска хода
                    if (((turn == 0 && Player_1->Skips != 3) || turn == 1 && Player_2->Skips != 3) && Second_chance()) {  //  Подтвержден
...
void Notepad(Player* Player) {
    ...
    //  Количество слов у игрока в блокноте
    ...
    while (1) {
        ...
        if (Player->Notepad[current_word][0] != 0) {  //  Есть слово
            printf(" %s ", Player->Notepad[current_word]);  //  Подсвечиваемое слово
        }
        else {  //  Пустое поле для добавления
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_UP && current_word != 0) {  //  Вверх
            ...
            if (code == KEY_DOWN && current_word < 8) {  //  Вниз
                //  Убираем окрас с предыдущей подсвечиваемой функции и меняем подсвет функции ниже
                gotoxy((short)((con_width_new() - 120) / 2 + strlen("   |  |  |                                   +---+ +---+ +---+ +---+ +---+                       | ")),
                       (short)((con_height_new() - 30) / 2 + 9 + current_word));
                if (Player->Notepad[current_word][0] != 0) {
                    printf(" %s ", Player->Notepad[current_word]);
                }
                else {
                    printf("                     ");
                }
            ...
            if (code == KEY_ENTER) {  //  Выбор
                if (Player->Notepad[current_word][0] != 0) {  //  Выбрано слово
                    ...
                    //  Цикл для считывания выбора пользователя
                    while (1) {
                        if (!key_is_pressed()) {
                            int code_del = key_pressed_code();
                            if (code_del == KEY_ENTER) {  //  Удалить
                                Player->Notepad[current_word][0] = 0;
                                //  Стираем с таблицы
                                gotoxy((short)((con_width_new() - 120) / 2 + strlen("   |  |  |                                   +---+ +---+ +---+ +---+ +---+")),
                                       (short)((con_height_new() - 30) / 2 + 9 + current_word));
                                printf("                       |                      ");
                            ...
                            if (code_del == KEY_ESC) {  //  Отмена
                                break;
                            }
                        }
                    }
                    continue;
                } else {  //  Выбрано поле добавления
                    ...
                    //  Проверка отмены
                    if (!strcmp((char*)word, "---")) {  //  Если отмена
                        //  Стираем надписи
                        gotoxy((short)((con_width_new() - 120) / 2 + strlen("   |  |  |                                   +---+ +---+ +---+ +---+ +---+")),
                               (short)((con_height_new() - 30) / 2 + 9 + current_word));
                        printf("                       |                      ");
                    ...
                    if (strlen((char*)word) <= 15) {  //  Запись
                        ...
                        gotoxy((short)((con_width_new() - 120) / 2 + strlen("   |  |  |                                   +---+ +---+ +---+ +---+ +---+   ")),
                               (short)((con_height_new() - 30) / 2 + 9 + current_word));
                        con_set_color(CON_CLR_RED, CON_CLR_BLACK);
                        printf("Слишком длинное    ");  //  Вывод ошибки
                        con_set_color(CON_CLR_WHITE_LIGHT, CON_CLR_BLACK);
                        Sleep(2000);
                        //  Стираем и отменяем
                        gotoxy((short)((con_width_new() - 120) / 2 + strlen("   |  |  |                                   +---+ +---+ +---+ +---+ +---+")),
