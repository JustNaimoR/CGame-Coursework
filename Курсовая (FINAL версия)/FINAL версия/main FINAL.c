...
struct Main_Data {
    ...
    int Last_Game;  //  Значение наличия незавершенной игры с компьютером
} Main_Data;
...
//  Данные прошлой сессии игры против бота  =====================================================================  4  ==
struct Last_Game {
    Player* Player;  //  Данные по игроку
    int Points_of_COMP;  //  Игровые очки компьютера
    unsigned char Playing_filed[5][5];
    List* BeginOfList;  //  История слов в игре
    Difficulty_level difficult;  //  Сложность игры
};
//  Адрес на струткуру прошлой сессии
struct Last_Game* link;
//  =============================================================================================================  4  ==
...
void Game_COMP(Difficulty_level difficulty, unsigned char* first_word, Player* Player_1, Player* Player_2);
unsigned char* COMP_COMPStep(unsigned char playing_field[5][5], Difficulty_level difficult, unsigned char first_word[6]);
void TwoPlayersGame(unsigned char* first_word, Player* Player_1, Player* Player_2);
//  =============================================  Интерфейс игры  =====================================================
void Game_Interface(unsigned char playing_field[5][5], int turn, Current_option option, int x, int y, struct Player* Player_1, struct Player* Player_2);
//  ============================================  Префиксное дерево  ===================================================
...
unsigned char** Game_Words(unsigned char playing_field[5][5]);
int Game_CheckAddingWord(unsigned char* word, unsigned char playing_field[][5], int option);
int Game_WordInField(unsigned char* word, int mark, unsigned char playing_field[5][5], int X, int Y, int hole, int option);
int Game_CheckLastElements(int X, int Y);
...
void LAST_GAMEFile_OpenTheData();
...
void OptionPlay_PrePlay_View(int option) {
    ...
    //  Добавление последней строки "Продолжить последнюю" в случае незавершенной прошлой игры
    if (Main_Data.Last_Game == 1) {
        gotoxy((short) ((width_of_console - width_of_table) / 2),
               (short) ((height_of_console - height_of_table) / 2 + 5));  //  Строка опции
        printf("| ");
        con_set_color(CON_CLR_MAGENTA_LIGHT, CON_CLR_BLACK);
        printf("Продолжить последнюю");
        con_set_color(CON_CLR_WHITE_LIGHT, CON_CLR_BLACK);

        printf("  |");

        gotoxy((short) ((width_of_console - width_of_table) / 2),
               (short) ((height_of_console - height_of_table) / 2 + 6));  //  Конец таблички
        printf("%s", play_gameplay[11]);
    }
}
    ...
    while (1) {
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_ENTER) {  //  Выбор
                ...
                else if (current_option == 2) {  //  Низкая сложность
                    Pre_Game(EASY);
                }
                break;
            }
...
void Pre_Game(Difficulty_level difficulty) {
    ...
    //  При начале новой игры - записанная ранее стирается
    Main_Data.Last_Game = 0;
    ...
    if (Root.elements[1] != NULL) {  //  Если добавлены слова
        ...
        if (difficulty == TWO_PLAYERS) {  //  Игра с человеком
            ...
            Player* Player_1 = (Player*) malloc((sizeof (Player)));
            Player_1 -> Name = (unsigned char*) "Игрок";    //  Имя
            Player_1 -> Skips = 0;                          //  Количество пропусков
            Player_1 -> Number_of_points = 0;               //  Количкство очков
            memset(Player_1 -> Notepad, 0, sizeof (Player_1 -> Notepad));
            //  Создаем структуру компьютера
            Player* Player_2 = (Player*) malloc((sizeof (Player)));
            Player_2 -> Name = (unsigned char*) "COMP";     //  Имя
            Player_2 -> Skips = 0;                          //  Количество пропусков
            Player_2 -> Number_of_points = 0;               //  Количкство очков
            memset(Player_2 -> Notepad, 0, sizeof (Player_2 -> Notepad));  //  Обнуление блокнота (но по сути не играет роли)
            //  >>  Готовы два игрока

            Game_COMP(difficulty, first_word, Player_1, Player_2);
        }
    } else {  //  Если слов нет
...
void Game_COMP(Difficulty_level difficulty, unsigned char* first_word, Player* Player_1, Player* Player_2) {
    ...
    if (!Main_Data.Last_Game) {  //  Если включена игра с самого начала
    ...
    } else if (Player_1 == NULL && Player_2 == NULL){  //  Включено продолжение игры
        //  Заполняем все поле прошлыми значениями
        for (int mark_X = 0, mark_Y = 0; ; mark_X++) {
            if (mark_X == 5) {
                mark_X = 0;
                mark_Y++;
            }
            ...
            playing_field[mark_X][mark_Y] = link->Playing_filed[mark_X][mark_Y];
        }
        //  Восстанавливаем игрока
        Player_1 = link->Player;
        //  Работаем со структурой компьютера
        Player_2 = (Player*) malloc(sizeof(Player));
        Player_2->Number_of_points = link->Points_of_COMP;
        Player_2->Name = (unsigned char*) "COMP";
        //  >>  Записаны оба игрока и игра продолжается
    }
    ...
    //  Вызываем интерфейс
    Game_Interface(playing_field, turn, current_option, 2, 2, Player_1, Player_2);
    ...
    while (1) {
        ...
        if (turn == 2) {  //  Если на данные момент ход компьютера
            Sleep(2000);  //  Задержка - типа компьютер думает, но на самом деле он гараздо быстрее тебя... Без шансов, парень
            //  Получаем выбранное компьютером слово
            unsigned char* COMP_word = COMP_COMPStep(playing_field, difficulty, first_word);
            //  Записываем выбранное слово на игровое поле
            Game_CheckAddingWord(COMP_word, playing_field, 1);
            number_of_cells--;  //  Уменьшаем количество доступных ячеек

            //  Начисляем очки за слово
            Player_2->Number_of_points += (int) strlen((char*) COMP_word);

            //  Добавление слова в список
            Game_AddWordInList(COMP_word);
            ...
            Game_Interface(playing_field, turn, current_option, current_X, current_Y, Player_1,
                           Player_2);
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_UP) {  //  Передвижение вверх
                if (current_Y > 0 && current_option == PLAY_FIELD) {  //  Передвижение по игровому полю
                    ...
                    continue;
                } else if (current_option == CHECK_WORD) {  //  Переход на опцию "блокнот" с пропуска хода
                    ...
                    while (1) {
                        ...
                        if (!key_is_pressed()) {
                            ...
                            if (code_2 == KEY_ENTER) {  //  Подтверждение слова
                                ...
                                //  Начисляем очки за слово (Здесь начисляться очки могут лишь игроку, поскольку бот обрабатывается в другом месте)
            ...
            if (code == KEY_ENTER) {
                if (current_option == PLAY_FIELD) {  //  Выбор ячейки на игровом поле
                    ...
                    if ((turn == 0 && Player_1->Skips != 3) && Game_SecondChance_SKIP()) {  //  Подтвержден и доступен
                        if (Player_1->Skips != 3) {
                            Player_1 -> Skips++;
                            turn = 2;  //  Меняем очередь
                        }
                        mode = 0;
                        ...
                        current_Y = -1;
                        current_option = PLAY_FIELD;
                        Game_Interface(playing_field, turn, current_option, current_X, current_Y, Player_1, Player_2);
            ...
            if (code == KEY_ESC) {
                if (mode == 1) {  //  Если стоит мод на выставление слова
                    ...
                    if (Game_SecondChance_ESC()) {
                        ...
                        Main_Data.Last_Game = 1;
                        //  Сохранение данных прошлой игры в файле LAST_GAME
                        FILE* LastGameFile = fopen("LAST_GAME.txt", "w");
                        //  Заполнение файла LAST_GAME.txt
                        //  Информация об игроке
                        fprintf(LastGameFile, "Skips %d\nNumber_of_points %d\nNotepad\n", Player_1->Skips, Player_1->Number_of_points);
                        for (int string = 0; string < 9; string++) {
                            if (Player_1->Notepad[string][0] != 0) fprintf(LastGameFile, "    %d %s\n", string, Player_1->Notepad[string]);
                        }
                        fprintf(LastGameFile, "    |\n");
                        //  Игровое поле
                        for (int mark_X = 0, mark_Y = 0; ; mark_X++) {
                            if (mark_X == 5) {
                                fprintf(LastGameFile, "\n");
                            ...
                            fprintf(LastGameFile, "%d", playing_field[mark_X][mark_Y]);
                        }
                        //  Очки компьютера
                        fprintf(LastGameFile, "Points_of_COMP %d\n|\n", Player_2->Number_of_points);
                        //  История слов
                        List* dop_mark = BeginOfList;
                        while (dop_mark->next != NULL) {
                            fprintf(LastGameFile, "%s\n", dop_mark->word);
                            dop_mark = (List *) dop_mark->next;
                        }
                        fprintf(LastGameFile, "|\n");
                        //  Сложность игры
                        fprintf(LastGameFile, "Difficulty_level %d\n", difficulty);
...
unsigned char* COMP_COMPStep(unsigned char playing_field[5][5], Difficulty_level difficult, unsigned char first_word[5]) {
    ///  Функция хода бота (Выходное значение -- его выбранное слово)
    /*  Алгоритм работы:
     *      1. Выделяется 1 слово из словаря
     *      2. Пытаемся его поместить на поле
     *      3.  Слово может быть выставлено? ==>
     *              добавляем его в дополнительный словарь и начинаем процесс заново
     *          Слово не может быть выставлено? ==>
     *              начинаем процесс заново (до момента проверки последнего слова) */

    //  Получаем список возможных слов
    unsigned char** words = Game_Words(playing_field);

    for (int current_word = 0; words[current_word][0] != 0; current_word++) {
        //  Проверка на присутсвие ранее слова
        if (Game_RepeatCheck(words[current_word]) || !strcmp((char*) words[current_word],(char*) first_word)) {
            int last_word_mark = 0;
            //  Добенаем до конца массива ( >> last_word_mark - номер конца массива)
            while (words[last_word_mark][0] != 0) {
                last_word_mark++;
            }
            //  Записываем последнее слово в ячейку, где было встречено использованное ранее слово
            strcpy((char*) words[current_word], (char*) words[last_word_mark - 1]);
            words[last_word_mark - 1][0] = 0;  //  Закрываем предыдущее
            current_word--;  //  Для проверки выставленного слова вместо убранного (может он тоже не должен находиться здесь)
        }
    }
    //  >>  Список words - все возможные слова для использования ботом (С учетом использованных ранее)

    //  Подсчет количества различных длин слов
    int koll[25];  //  Массив хранения ( индекс - длина слова)
    memset(koll, 0, sizeof (koll));

    for (int mark = 0; words[mark][0] != 0; mark++) {
        koll[strlen((char*) words[mark])]++;  //  Увеличиваем количество в ячейке
    }
    //  >>  Набрали количество каждого из элементов

    /* Процентные соотношения выбора слова для каждой из сложностей: (X - длина слова)
     *        EASY                NORMAL                HARD
     *    20%  -  X == 2|3    8%   -  X == 2|3     5%   -  X == 2|3
     *    45%  -  X == 4      17%  -  X == 4       5%   -  X == 4
     *    30%  -  X == 5      50%  -  X == 5|6     30%  -  X == 5|6|7
     *    5%   -  X == 6      20%  -  X == 7|8     30%  -  X == 8|9|10
     *                        5%   -  X == 9       20%  -  X == 11
     *                                             10%  -  X >= 12
     * */

    // Дополнительные 10 прокруток значения Random
    for (int mark = 0; mark < 10; mark++) {
        Main_Data.Random = LKG(Main_Data.Random);
    }

    if (difficult == EASY) {  //  Выбрана сложность "легко"
        while (1) {  //  Цикл для случаев выпадения пустого диапазона
            //  Просмотр выпавшего числа
            if (Main_Data.Random % 100 <= 19 && koll[2] + koll[3] != 0) {  //  20%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);

                //  Побор слова из выбранного диапазона
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % (koll[2] + koll[3]); last_mark++) {
                    if (strlen((char *) words[last_mark]) == 2 || strlen((char *) words[last_mark]) == 3) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку

                if (last_mark != 0) {  //  Проверка
                    last_mark--;
                }
                return words[last_mark];
            } else if (Main_Data.Random % 100 > 19 && Main_Data.Random % 100 < 65 && koll[4] != 0) {  //  45%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % koll[4]; last_mark++) {
                    if (strlen((char *) words[last_mark]) == 4) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 65 && Main_Data.Random % 100 < 95 && koll[5] != 0) {  //  30%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % koll[5]; last_mark++) {
                    if (strlen((char *) words[last_mark]) == 5) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 > 94 && koll[6] != 0) {  //  5%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % koll[6]; last_mark++) {
                    if (strlen((char *) words[last_mark]) == 6) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else {  //  Случай, когда в выпавшем диапазоне нет чисед с заданной длиной
                Main_Data.Random = LKG(Main_Data.Random);
                continue;
            }
        }
    } else if (difficult == NORMAL) {  //  Выбрана сложность "нормально"
        while (1) {  //  Цикл для случаев выпадения пустого диапазона
            //  Просмотр выпавшего числа
            if (Main_Data.Random % 100 >= 72 && Main_Data.Random % 100 < 95 && koll[2] + koll[3] != 0) {  //  13%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 20 && Main_Data.Random % 100 < 32 && koll[4] != 0) {  //  12%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 32 && Main_Data.Random % 100 < 72 && koll[5] + koll[6] != 0) {  //  50%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % (koll[5] + koll[6]); last_mark++) {
                    if (strlen((char *) words[last_mark]) == 5 || strlen((char *) words[last_mark]) == 6) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 0 && Main_Data.Random % 100 < 20 && koll[7] + koll[8] != 0) {  //  20%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % (koll[7] + koll[8]); last_mark++) {
                    if (strlen((char *) words[last_mark]) == 7 || strlen((char *) words[last_mark]) == 8) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 95 && koll[9] != 0) {  //  5%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % koll[9]; last_mark++) {
                    if (strlen((char *) words[last_mark]) == 9) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
    ...
    } else if (difficult == HARD) {  //  Выбрана сложность "сложно"
        while (1) {  //  Цикл для случаев выпадения пустого диапазона
            //  Просмотр выпавшего числа
            if (Main_Data.Random % 100 < 5 && koll[2] + koll[3] != 0) {  //  5%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 5 && Main_Data.Random % 100 < 10 && koll[4] != 0) {  //  5%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 10 && Main_Data.Random % 100 < 40 && koll[5] + koll[6] + koll[7] != 0) {  //  30%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % (koll[5] + koll[6] + koll[7]); last_mark++) {
                    if (strlen((char *) words[last_mark]) == 5 || strlen((char *) words[last_mark]) == 6 || strlen((char *) words[last_mark]) == 7) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку

                if (last_mark != 0 && strlen((char*) words[last_mark - 1]) == 5) {  //  Дополнительное условие для большего выпадения слов длиной 6/7
                    Main_Data.Random = LKG(Main_Data.Random);
                    if (Main_Data.Random % 10 < 5 && koll[7] != 0) {  //  Выпадение первой длины 7 знаков
                        last_mark = 0;
                        while (strlen((char*) words[last_mark++]) != 7);
                    } else if (Main_Data.Random % 10 >= 5 && Main_Data.Random % 10 < 8 && koll[6] != 0) {  //  Выпадение первой длины 6 знаков
                        last_mark = 0;
                        while (strlen((char*) words[last_mark++]) != 6);
                    } else if (Main_Data.Random % 10 >= 8 && koll[5] != 0) {  //  Выпадение первой длины 5 знаков
                        last_mark = 0;
                        while (strlen((char*) words[last_mark++]) != 5);
                    }
                }
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 40 && Main_Data.Random % 100 < 70 && koll[8] + koll[9] + koll[10] != 0) {  //  30%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % (koll[8] + koll[9] + koll[10]); last_mark++) {
                    if (strlen((char *) words[last_mark]) == 8 || strlen((char *) words[last_mark]) == 9 || strlen((char *) words[last_mark]) == 10) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 70 && Main_Data.Random % 100 < 90 && koll[11] != 0) {  //  20%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % koll[11]; last_mark++) {
                    if (strlen((char *) words[last_mark]) == 11) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
                ...
                return words[last_mark];
            } else if (Main_Data.Random % 100 >= 90 && koll[12] != 0) {  //  10%
                //  Обновляем значение Random
                Main_Data.Random = LKG(Main_Data.Random);
                ...
                int last_mark;
                int current_num = 0;  //  Текущее количество пройденных слов, входящих в заданный диапазон
                for (last_mark = 0; current_num != Main_Data.Random % koll[12]; last_mark++) {
                    if (strlen((char *) words[last_mark]) == 12) {
                        current_num++;
                    }
                }  //  >>  Получаем current_num - номер выбранного слова по списку
...
void Game_Interface(unsigned char playing_field[5][5], int turn, Current_option option, int x, int y, struct Player* Player_1, struct Player* Player_2) {
    ...
    ///    turn - чей сейчас ход (turn == 0 - ход первого игрока, 1 - второго, 2 - xод компьютера)
    ...
    unsigned char players_turn[3][50] = { " ==  Ход Игрока 1  == "," ==  Ход Игрока 2  == "," == Ход компьютера == " };
    ...
    if (turn == 0) {  //  Ход первого игрока
        ...
        con_set_color( CON_CLR_GRAY, CON_CLR_BLACK);  //  Синий шрифт и черный фон
    }
    ...
    gotoxy((short) ((width_of_console - width_of_field) / 2 + strlen("   Рейтинг                                                                                       |  ")),
           (short) ((height_of_console - height_of_field) / 2 + 1));
    if (!strcmp((char*) Player_2 -> Name, "COMP")) {  //  Если играет игрок и компьютер
    ...
    } else if (turn == 1) {  //  Слова второго игрока
        for (int string = 0; string != 9; string++) {
            gotoxy((short)((width_of_console - width_of_field) / 2 + strlen("                                             +---+ +---+ +---+ +---+ +---+                       | ")),
                   (short)((height_of_console - height_of_field) / 2 + 9 + string));
            printf(" %s", Player_2->Notepad[string]);
        }
    } else {  //  Слова компьютера
        for (int string = 0; string != 9; string++) {
            gotoxy((short)((width_of_console - width_of_field) / 2 + strlen("                                             +---+ +---+ +---+ +---+ +---+                       | ")),
                   (short)((height_of_console - height_of_field) / 2 + 9 + string));
            printf("      -- -- --");
    ...
    } else {  //  Компьютер (стираем надпись пропущенных ходов)
        gotoxy((short) ((width_of_console - width_of_field) / 2),
    ...
    if (option == SKIP) {  //  Подсвечивать функцию пропуска
        ...
        if (x == -1 && y == -1) {  //  Ход клмпьютера - остутсвует подсветка
            con_set_color(CON_CLR_WHITE_LIGHT, CON_CLR_BLACK);
            return;
        }
...
void OptionPlay() {
    ...
    if (Main_Data.Last_Game) {  //  Если ранее была незакончена игра
        //  Вызываем операцию записи данных об прошлой игре в структуру
        LAST_GAMEFile_OpenTheData();
    }
    ...
    while (1) {
        ...
        if (!key_is_pressed()) {
            ...
            if (code == KEY_UP) {
                if (current_option == 0) {  //  Если уже потолок опций
                    ...
                    if (current_option != 2) {
                        ...
                        con_set_color(CON_CLR_MAGENTA_LIGHT, CON_CLR_BLACK);
                        gotoxy((short) ((con_width_new() - width_of_table) / 2 + strlen("| ")),
                               (short) ((con_height_new() - height_of_table) / 2 + 5));
                        printf("Продолжить последнюю");
                    }
                    //  Переходим к новой строке и выделяем ее
                    Sleep(40);
                    ...
                    if (current_option == 0) {
                        gotoxy((short) ((con_width_new() - width_of_table) / 2 + strlen("|  ")),
                               (short) ((con_height_new() - height_of_table) / 2 + 3 + current_option));
                    } else if (current_option == 1) {
                        gotoxy((short) ((con_width_new() - width_of_table) / 2 + strlen("|  ") + 4),
                               (short) ((con_height_new() - height_of_table) / 2 + 3 + current_option));
                    }
                    con_set_color(CON_CLR_BLACK, CON_CLR_WHITE_LIGHT);  //  Выделение (черные буквы и белый фон)
                    printf("%s", option_schemes[current_option]);
            ...
            if (code == KEY_DOWN) {
                if (current_option == 1 && Main_Data.Last_Game != 1 || Main_Data.Last_Game == 1 && current_option == 2) {  //  Если ниже некуда
                    continue;
                } else if (current_option == 1 && Main_Data.Last_Game == 1) {  //  Если была незакончена игра (3-я функция)
                    //  Выводим снова текущую выделенную строку с дефолтным цветом (== Стираем выделение)
                    con_set_color(CON_CLR_WHITE_LIGHT, CON_CLR_BLACK);
                    gotoxy((short)((con_width_new() - width_of_table) / 2 + strlen("|  ") + 4),
                           (short)((con_height_new() - height_of_table) / 2 + 4));
                    ...
                    Sleep(40);
                    current_option++;
                    gotoxy((short) ((con_width_new() - width_of_table) / 2 + 2),
                           (short) ((con_height_new() - height_of_table) / 2 + 5));
                    con_set_color(CON_CLR_GRAY, CON_CLR_BLACK);
                    printf("Продолжить последнюю");
                    con_set_color(CON_CLR_WHITE_LIGHT, CON_CLR_BLACK);  //  Возвращаем изначальные настройки цветов
            ...
            if (code == KEY_ENTER) {  //  Выбранная опция
                ...
                if (current_option == 2) {  //TODO  Продолжение прошлой игры
                    if (Main_Data.Animation) {  //  Анимация
                        Sleep(500);  //  Задержка
                        clrscr();
                        Sleep(1000);  //  Задержка
                        MainMenu_Animation_Loading();
                    }
                    //  Вызов функции игры с пустыми значениями ячеек (все изменится в самой функции игры)
                    ...
                    Game_COMP(link->difficult, str, NULL, NULL);
                }
            }
            if (code == KEY_ESC) {  //  Выход
                return;
...
int LKG(int seed) {
    ...
    return ((int)(16807 * seed + 0) /*% (int)pow(2, 31))*/ % 1000);
}
...
//  =======================================  Обслуживающие функции игры  ===============================================
unsigned char** Game_Words(unsigned char playing_field[5][5]) {
    ///  Функция создания возможных слов для хода компьютера (playing_field[][] - игровое поле)
    ///      (Возвращает созданный список слов - двумерный массив)

    //  Дополнительный адрес для прохода по дереву
    Block* dop_link = (Block*) Global_root;

    //  Массив адресов элементов узла (букв) текущего добавляемого слова
    Element* current_word[40];
    memset(current_word, 0, sizeof (current_word));
    //  Порядковый номер текущей выставляемой буквы
    int num_of_symbol = 0;

    //  Массив порядковых номеров текущих выбранных адресов в узлах дерева (а также и номер узла дерева)
    int num_of_link[50];
    //  Обнуляем массив (Не забываем, что нулевой элемент в узле - элемент заглушка)
    for (int mark = 0; mark < 50; mark++) { num_of_link[mark] = 1; }

    //  Массив подходящих слов для бота (создание с помощью malloc)
    unsigned char** correct_words = (unsigned char**) malloc(sizeof (unsigned char**) * 200);
    memset(correct_words, 0, sizeof (unsigned char) * 200);
    //  Создание массивы адресов для двумерного массива
    for (int mark = 0; mark < 200; mark++) {
        unsigned char* string = (unsigned char*) malloc(sizeof (unsigned char*) * 50);
        memset(string, 0, sizeof (unsigned char*) * 50);
        correct_words[mark] = string;
    }

    //  Цикл создания слов
    while (1) {

        //  Проверка на существование символов в данном узле
        if (dop_link->elements[num_of_link[num_of_symbol]] != NULL) {  //  Существует продолжение ветки

            //  Записываем адрес очередной буквы
            current_word[num_of_symbol] = dop_link->elements[num_of_link[num_of_symbol]];
            dop_link = dop_link->elements[num_of_link[num_of_symbol]]->next;  //  Переходим к узлу по пути выставленного символа

            num_of_link[num_of_symbol]++;  //  Увеличиваем значение следующего выбираемого адреса в данном узле
            num_of_symbol++;  // Увеличиваем текущий номер заполняемого символа + меняем узел
        }  else if (dop_link->elements[0] != NULL) {  //  Ветка закончена (дошли до конца слова)

            //for (int mark = 0; current_word[mark] != 0; mark++) {  printf("%c", (unsigned char) current_word[mark]->symbol_code);  }  printf("\n");

            //  Создаем массив символов текущего слова
            unsigned char current_word_plus[40];
            memset(current_word_plus, 0, sizeof (current_word_plus));
            //  Запись символов (точнее - их числовых значений) в слово
            for (int mark = 0; current_word[mark] != 0; mark++) {
                current_word_plus[mark] = current_word[mark]->symbol_code;
            }

            //  Проверка на возможность подстановки слова на поле
            if (Game_CheckAddingWord(current_word_plus, playing_field, 0)) {  //  Если слово можно расположить
                //  Доходим до конца списка слов
                int num_of_word = 0;
                while (correct_words[num_of_word][0] != 0) {
                    num_of_word++;
                }
                //  Записываем слово
                strcpy((char*) correct_words[num_of_word], (char*) current_word_plus);
            }

            //  Переход к новому слову по списку
            num_of_link[num_of_symbol] = 1;  //  Поскольку мы покинули этот узел ветки (обнуляем позиции данной части)
            num_of_symbol--;  //  Уменьшаем значение заполняемого символа
            current_word[num_of_symbol] = 0;  //  Удаляем последний записанный символ
            //  Меняем адрес текущего узла на предыдущий узел
            dop_link = current_word[num_of_symbol - 1]->next;
        }  else  {  //  Когда данный узел исчерпал свои элементы

            //  Проверка на исчерпание корневого узла
            if (dop_link->elements[num_of_link[num_of_symbol]] == 0 && num_of_symbol == 0) {
            ...
            if (num_of_symbol != 0) {
                dop_link = current_word[num_of_symbol - 1]->next;
            } else {  //  Случай выпадения корневого узла
                dop_link = Global_root;
                //num_of_link[0]++;  //  Передвигаемся в корне узла
            }
...
//  Предыдущие использоыванные ячейки (координаты) в создаваемом слове ( для работы функции Game_WordInField() )
int last_elements[40][2];
int next_coordinates;
int Game_CheckAddingWord(unsigned char* word, unsigned char playing_field[][5], int option) {
    ///  Функция проверки подстановки слова на игровое поле
    ///    Делится на два этапа - (1) поиск первой буквы и поиск воззможности подстановки слова и (2) подстановка первой
    ///      с поиском варианта подстановки слова
    ///    option - при значении 1 - для слова будет добавлена буква ( то есть запись буквы для выбранного слова бота )

    //  Цикл пробега #1 по всему игровому полю в поисках первой буквы слова
    for (int mark_X = 0, mark_Y = 0; mark_Y < 5; mark_X++) {
        //  Найдено совпадение с первой буквой
        if (playing_field[mark_X][mark_Y] == word[0]) {
            int dop_X = mark_X, dop_Y = mark_Y;
            //  Обнуляем массив использованных координат
            next_coordinates = 0;
            for (int mark = 0; mark < 40; mark++) {
                last_elements[mark][0] = -1;
                last_elements[mark][1] = -1;
            }

            if (Game_WordInField(word, 1, playing_field, mark_X, mark_Y, 0, option) == strlen((char*) word)) {
                //  Слово можно выставить
                return 1;
            }
            //  В случае невозможности поставления слова относительно найденно буквы >> продолжается поиск и дальнейшая работа функции
        }

        //  Условие прохода на следующую строку матрицы-поля
        if (mark_X == 5) {
            mark_X = -1;
            mark_Y++;
        }
    }
    //  Цикл пробега #2 по всему игровому полю для составления слова на основе второго символа слова и последующей основной части
    for (int mark_X = 0, mark_Y = 0; mark_Y < 5; mark_X++) {
        //  Условие проверки (Если найдена вторая буква на поле + с одной из сторон есть пустая ячейка)
        if (playing_field[mark_X][mark_Y] == word[1] && (mark_Y != 0 && playing_field[mark_X][mark_Y - 1] == 0 ||
                mark_Y != 4 && playing_field[mark_X][mark_Y + 1] == 0 || mark_X != 4 && playing_field[mark_X + 1][mark_Y] == 0 ||
                    mark_X != 0 && playing_field[mark_X - 1][mark_Y] == 0)) {
            //  Устраиваем вторую проверку на выставление всего слова кроме 1 буквы
            int dop_X = mark_X, dop_Y = mark_Y;
            //  Обнуляем массив использованных координат
            next_coordinates = 0;
            for (int mark = 0; mark < 40; mark++) {
            ...
            if (Game_WordInField(word, 2, playing_field, mark_X, mark_Y, 1, 0) == strlen((char*) word)) {
                //  Слово можно выставить, используя пустую ячейку
                if (option) {  //  Если надо выставить данное слово на поле
                    if (mark_Y != 0 && playing_field[mark_X][mark_Y - 1] == 0) {  //  Ячейка сверху
                        playing_field[mark_X][mark_Y - 1] = word[0];
                    } else if (mark_Y != 4 && playing_field[mark_X][mark_Y + 1] == 0) {  //  Ячейка снизу
                        playing_field[mark_X][mark_Y + 1] = word[0];
                    } else if (mark_X != 4 && playing_field[mark_X + 1][mark_Y] == 0) {  //  Ячейка справа
                        playing_field[mark_X + 1][mark_Y] = word[0];
                    } else if (mark_X != 0 && playing_field[mark_X - 1][mark_Y] == 0) {  //  Ячейка слева
                        playing_field[mark_X - 1][mark_Y] = word[0];
                    }
                }
                return 1;
    ...
    //  На поле данное слово не выставить
    return 0;
}

int Game_WordInField(unsigned char* word, int mark, unsigned char playing_field[5][5], int X, int Y, int hole, int option) {
    ///  Рекурсивная функция поиска букв слова
    ///     X,Y - текущие рассматриваемые координаты поля под символ word[mark] (ищется именно это символ)
    ///     hole - значение использования пустой ячейки (0 - не использован, 1 - использован)
    ///     option - при значении 0 - функция лишь показывает есть ли возможность выставить слово, 1 - еще и записывает его
    ///         (Для выставления уже выбранного компьютером слова)
    //  Главное терминальное условие проверки выходного значения
    if (mark == strlen((char*) word)) {  //  Условие выполнятеся лишь в случае нахождения последовательности для записываемого слова
        if (!hole) {  //  Если слово было расположено, но новых букв не было выставленно ==> Слово не расположить
            return 0;
        }
        return mark;
    }

    //  Верхний элемент (нахождение элемента сверху или свободной ячейки в случае не использования пустых ячеек раньше)
    if (Y != 0 && (playing_field[X][Y - 1] == word[mark] ||
                   playing_field[X][Y - 1] == 0 && !hole) && !Game_CheckLastElements(X, Y - 1)) {
        last_elements[next_coordinates][0] = X;  //  Записываем в историю значение X
        last_elements[next_coordinates][1] = Y;  //  Записываем в историю значение Y
        next_coordinates++;
        if (playing_field[X][Y - 1] == 0 && !hole) {  //  Если сверху пустая ячейка и у нас есть возможность ее использовать (hole)
            hole = 1;  //  Пустая ячейка больше не должна использоваться
            //  Идем к этому элементу и проверяем все остальное относительно него + проверка на выставления всего слова (в таком случае выполянется условие)
            if (Game_WordInField(word, mark + 1, playing_field, X, Y - 1, hole, option) == strlen((char*) word)) {
                //  Условие выше выполняется лишь когда слово можно расположить на игровом поле
                if (option) {  //  Если установлена опция записи слова
                    //  Записываем в пустую ячейку букву слова
                    playing_field[X][Y - 1] = word[mark];
                }
                return (int) strlen((char*) word);
            }
            hole = 0;  //  После использования промежутка в одной ветке
        } else {  //  Сверху подходящий элемент
            if (Game_WordInField(word, mark + 1, playing_field, X, Y - 1, hole, option) == strlen((char*) word)) {
                return (int) strlen((char*) word);
            }
        }
        last_elements[next_coordinates - 1][0] = 0;  //  Удаление из истории значения записанного X
        last_elements[next_coordinates - 1][1] = 0;  //  Удаление из истории значения записанного Y
        next_coordinates--;
    }
    //  Правый элемент
    if (X != 4 && (playing_field[X + 1][Y] == word[mark] ||
                   playing_field[X + 1][Y] == 0 && !hole) && !Game_CheckLastElements(X + 1, Y)) {
        last_elements[next_coordinates][0] = X;  //  Записываем в историю значение X
        last_elements[next_coordinates][1] = Y;  //  Записываем в историю значение Y
        next_coordinates++;
        if (playing_field[X + 1][Y] == 0 && !hole) {  //  Если справа пустая ячейка и у нас есть возможность ее использовать (hole)
            hole = 1;  //  Пустая ячейка больше не должна использоваться
            if (Game_WordInField(word, mark + 1, playing_field, X + 1, Y, hole, option) == strlen((char*) word)) {
                //  Условие выше выполняется лишь когда слово можно расположить на игровом поле
                if (option) {  //  Если установлена опция записи слова
                    //  Записываем в пустую ячейку букву слова
                    playing_field[X + 1][Y] = word[mark];
                }
                return (int) strlen((char*) word);
            }
            hole = 0;  //  После использования промежутка в одной ветке
        } else {  //  Справа подходящий элемент
            if (Game_WordInField(word, mark + 1, playing_field, X + 1, Y, hole, option) == strlen((char*) word)) {
                return (int) strlen((char*) word);
            }
        }
    ...
    //  Нижний элемент
    if (Y != 4 && (playing_field[X][Y + 1] == word[mark] ||
                   playing_field[X][Y + 1] == 0 && !hole) && !Game_CheckLastElements(X, Y + 1)) {
        last_elements[next_coordinates][0] = X;  //  Записываем в историю значение X
        last_elements[next_coordinates][1] = Y;  //  Записываем в историю значение Y
        next_coordinates++;
        if (playing_field[X][Y + 1] == 0 && !hole) {  //  Если снизу пустая ячейка и у нас есть возможность ее использовать (hole)
            hole = 1;  //  Пустая ячейка больше не должна использоваться
            if (Game_WordInField(word, mark + 1, playing_field, X, Y + 1, hole, option) == strlen((char*) word)) {
                //  Условие выше выполняется лишь когда слово можно расположить на игровом поле
                if (option) {  //  Если установлена опция записи слова
    ...
    //  Левый элемент
    if (X != 0 && (playing_field[X - 1][Y] == word[mark] ||
                   playing_field[X - 1][Y] == 0 && !hole) && !Game_CheckLastElements(X - 1, Y)) {
        last_elements[next_coordinates][0] = X;  //  Записываем в историю значение X
        last_elements[next_coordinates][1] = Y;  //  Записываем в историю значение Y
        next_coordinates++;
        if (playing_field[X - 1][Y] == 0 && !hole) {  //  Если снизу пустая ячейка и у нас есть возможность ее использовать (hole)
            hole = 1;  //  Пустая ячейка больше не должна использоваться
            if (Game_WordInField(word, mark + 1, playing_field, X - 1, Y, hole, option) == strlen((char*) word)) {
                //  Условие выше выполняется лишь когда слово можно расположить на игровом поле
                if (option) {  //  Если установлена опция записи слова
...
int Game_CheckLastElements(int X, int Y) {
    ///  Функция проверки с прошлыми записанными коорднатами (записаны в last_elements[]) ( обсулживающая функции Game_WordInField() )
    for (int mark = 0; last_elements[mark][0] != -1 && mark != 40; mark++) {
        if (last_elements[mark][0] == X && last_elements[mark][1] == Y) {  //  Если уже были использованы эти координаты прежде
            return 1;  //  Координаты уже использовались
        }
    }
    return 0;  //  Координаты не использованы прежде
}
...
void Game_EndOfGame(Player* Player_1, Player* Player_2) {
    ...
    if (Player_1->Number_of_points > Player_2->Number_of_points) {  //  Победил первый
        ...
        con_set_color(CON_CLR_WHITE_LIGHT, CON_CLR_BLACK);
    } else if (!strcmp((char*) Player_2->Name, "Игрок 2")) {  //  Победил второй
        con_set_color(CON_CLR_BLUE_LIGHT, CON_CLR_BLACK);
    ...
    Sleep(3000);
    if (!(strcmp((char*)Player_2->Name, "COMP") == 0 && Player_2->Number_of_points > Player_1->Number_of_points)) {   //  Если играл не бот
...
int DataFile_CheckFile(const char* Data) {
    ...
    //  Проверка #4 ("Last_Game значение") - четвертая строка
    mark = 0;  //  Обнуляем для повторной работы
    mark_for_string = 0;  //  Обнуляем для повторной работы
    memset(current_string, 0, sizeof(current_string));  //  Обнуляем для повторной работы

    point = strstr(Data,"Last_Game");  //  Получение адреса первого вхождения
    if (point == NULL) {  //  Если не был найден
        return 0;
    }
    while (*(point + mark - 1) != ' ' && *(point + mark) != '\0') {  //  Запись в current_string метки (для наглядности. Не является необходимым)
...
void DataFile_SaveDataInFile() {
    ...
    fprintf(file, "Random %d\nAnimation %d\nUser_dictionary %s\nLast_Game %d\n", Main_Data.Random, Main_Data.Animation, Main_Data.User_dictionary, Main_Data.Last_Game);
...
void DataFile_RecordTheDataInStruct(char* Data) {
    ...
    //  Запись значения Last_Game в струтурное поле
    point = strstr(Data, "Last_Game");
    Main_Data.Last_Game = GiveTheNumber(point + 10);
}
...
void LAST_GAMEFile_OpenTheData() {
    ///  Функция октрытия данных из файла предыдущей игры (LAST_GAME)

    FILE* LastGameFile = fopen("LAST_GAME.txt", "r");
    link = (struct Last_Game*) malloc(sizeof (struct Last_Game));
    //  Массив хранения файла LAST_GAME
    unsigned char LastGameData[400];
    memset(LastGameData, 0, sizeof (LastGameData));
    int mark = 0;

    while (LastGameFile && fscanf(LastGameFile, "%c", &LastGameData[mark++]) != EOF);  //  Запись файла
    mark = 0;  //  Номер участка файла LAST_GAME
    char* point;  //  Метка первого вхождения

    Player* Player_1 = (Player*) malloc(sizeof(Player));  //  Создание структуры
    Player_1 -> Name = (unsigned char*) "Player";  //  Дефолтное имя игрока

    point = strstr((char*) LastGameData,"Skips");  //  Количество пропусков
    Player_1 -> Skips = GiveTheNumber(point + 6);

    point = strstr((char*) LastGameData,"Number_of_points");  //  Количество очков
    Player_1 -> Number_of_points = GiveTheNumber(point + strlen("Number_of_points "));

    point = strstr((char*) LastGameData, "Notepad");  //  Слова в блокноте
    point = point + strlen("Notepad\n    ");  //  Переносимся к слову
    //  Очистка строк в блокноте
    for (mark = 0; mark < 9; mark++) {
        memset(Player_1->Notepad, 0, sizeof (Player_1->Notepad));
    }
    //  Цикл записи слов
    for ( ; *point != '|'; point += 2 + strlen(GiveTheString(point + 2)) + 1 + 4) {
        //  Копируем числовые значения слов
        for (int dop_mark = 0; point[dop_mark + 2] != '\n'; dop_mark++) {
            Player_1->Notepad[*point - 48][dop_mark] = *(point + dop_mark + 2);
        }
    }
    //  Записываем данные игрока в структуру
    struct Last_Game* LastGame = malloc(sizeof (struct Last_Game));
    LastGame->Player = Player_1;

    point += 2;  //  Пропускаем '\n' после слов из блокнота
    //  >>  Далее идет поле игры

    for (int mark_X = 0, mark_Y = 0; *point != '|'; mark_X++) {
        if (mark_X == 5) {
            mark_X = 0;
            mark_Y++;
        }
        LastGame->Playing_filed[mark_X][mark_Y] = GiveTheNumber(point);
        //  Переходим к следующему числу
        while (*point != ' ' && *point != '\n') {
            point++;
        }
        point++;  //  Пропускаем пробел
    }
    //  >>  Записано игровое поле

    //  Переходим к части игровых очков компьютера
    point = strstr((char*) LastGameData, "Points_of_COMP");
    point += strlen("Points_of_COMP ");
    LastGame->Points_of_COMP = GiveTheNumber(point);

    //  Запись использованных ранее слов
    point += 4;  //  Переход в нужную точку файла
    //  Создание начала списка использованных слов и заполнение использованными ранее
    BeginOfList = (List*) malloc(sizeof(List));
    BeginOfList -> word[0] = 0;  //  Закрываем слово
    BeginOfList -> next = NULL;  //  Закрываем его
    //  Записываем слова из файла
    while (*point != '|') {
        Game_AddWordInList((unsigned char*) GiveTheString(point));
        point += 1 + strlen(GiveTheString(point));  //  Пропуск '\n' по файлу
    }
    //  Запись списка в структуру
    LastGame->BeginOfList = BeginOfList;

    //  Запись сложности игры
    point = strstr((char*) LastGameData, "Difficulty_level");
    LastGame->difficult = GiveTheNumber(point + strlen("Difficulty_level "));
    //  Запись адреса на струткуру
    link = LastGame;
    //  Записаны все данные прошлой сессии:
    //    Все данные игрока, количество очков бота, игровое поле и список ранее использованных
    //      слов - все находится в структуре Last_Game
}
